...
ModuleResult TabularDescribe::apply(IData& data) {
    ...
    for (const auto& col : table.header()) {
        ...
        res.set(col + "_sum", sumMod.apply(table).get<double>(col + "_sum"));
        res.set(col + "_count", countMod.apply(table).get<int>(col + "_count"));
        res.set(col + "_mean", meanMod.apply(table).get<double>(col + "_mean"));
        res.set(col + "_std", stdMod.apply(table).get<double>(col + "_std"));
        res.set(col + "_min", minMod.apply(table).get<double>(col + "_min"));
        res.set(col + "_max", maxMod.apply(table).get<double>(col + "_max"));
        res.set(col + "_median", medianMod.apply(table).get<double>(col + "_median"));
        res.set(col + "_isnull", isNullMod.apply(table).get<int>(col + "_isnull"));
    }
